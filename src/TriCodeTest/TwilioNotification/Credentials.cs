using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace TriCodeTest.TwilioNotification
{
    /// <summary>
    /// 
    /// </summary>
    public class Credentials
    {
        /// <summary>
        /// Token ID to authenticate twilio to send message
        /// </summary>
        public static string AcctSID= "AC8b47dd8402193c27fdc523dd614c9c98";
        /// <summary>
        /// Token token to authenticate twilio to send message
        /// </summary>
        public static string AcctToken = "5aa7fc1c4d9ecbdf551d64d521440f97";
        /// <summary>
        /// Twilios varified number to send message. 
        /// </summary>
        public static string TwilioNumber = "+15096787994";

    }
}
